...
- 🚀 **Smart incremental deploys** using git-ftp (only uploads changed files)
- 📋 **Auto-detects config** from `.env`, `deploy.php`, `.ftpconfig`
- 🎯 **Per-project configuration** stored in `.cowboy-deploy.json`
- 🔄 **Deployment history** with git commit tracking
- ↩️ **Rollback support** to previous deployments
- 🏗️ **Asset building** (npm run build) before deployment
- 📦 **Smart composer install** (only when composer.lock changes)
...
**⚠️ Security:** This file contains FTP credentials! It's automatically added to `.gitignore`.
...
Made with ❤️ for hassle-free FTP deployments 🤠
